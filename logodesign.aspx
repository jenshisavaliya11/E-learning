<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="logodesign.aspx.cs" Inherits="logodesign" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">

<link href="css/logodesign.css?v=1.1" rel="stylesheet" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">


<div class="container">
    <div class="content-left">
      
      <h1>Advanced Logo Design</h1>
      <p class="description">
Logos are more than visuals — they’re the face of a brand. Our Advanced Logo Design Course is tailored for designers who want to go beyond the basics and master the art of creating impactful, timeless, and strategic brand marks. This course dives deep into design theory, creativity, and professional tools to help you design logos that stand out and tell powerful stories.</p>     
        <asp:Image ID="Image1" runat="server" ImageUrl="~/projectimg/logodesign.jpeg" Width="650px"   BorderStyle="Ridge"/>   
      </div>
    <div class="form-container">
    <div class="content-right">
      <div class="price">RS.  10,000</div>
      <p class="price-desc">
       
      > Enroll Now – Advanced Logo Design<br>

      
      </p>
      
     
      <ul class="info-list">
        <li>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M3 3h18v2H3zm3 7h12v2H6zm-3 7h18v2H3z"/></svg>
          Level: <b>Premium</b>
        </li>
        <li>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="none" stroke="#546a8f" stroke-width="2"/><path d="M12 6v6l4 2"/></svg>
          Duration: <b>7hr 24m</b>
        </li>
        <li>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><rect width="16" height="12" x="4" y="6" rx="2" ry="2" fill="none" stroke="#546a8f" stroke-width="2"/><path d="M8 10h8v4H8z"/></svg>
          Lessons: <b>50</b>
        </li>
        <li>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><polygon points="12 17.27 18.18 21 16.54 13.97 22 9.24 14.81 8.63 12 2 9.19 8.63 2 9.24 7.46 13.97 5.82 21"/></svg>
          Lifetime Access
        </li>
        <li>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><rect x="7" y="2" width="10" height="4" rx="1" ry="1" fill="none" stroke="#546a8f" stroke-width="2"/><path d="M7 6v14a2 2 0 0 0 4 0v-5h2v5a2 2 0 0 0 4 0V6"/></svg>
          Access From Any Computer, Tablet or Mobile
        </li>
        <button class="add-to-cart"> <a href="logoform.aspx">Enroll Now</a></button>
      </ul>
    </div>
    </div>
    
  </div>
  <div class="a10container">
  <div class="content" style="color:Gray">
    <h2>About the course</h2>
    <p>
Logos are more than visuals — they’re the face of a brand. Our Advanced Logo Design Course is tailored for designers who want to go beyond the basics and master the art of creating impactful, timeless, and strategic brand marks. This course dives deep into design theory, creativity, and professional tools to help you design logos that stand out and tell powerful stories.    </p>

    <div class="divider"> 

    <h2>What will you learn</h2>
    <p>🔹 Advanced Design Principles – Shapes, symbols, and the psychology of colors in branding<br>

🔹 Creative Concept Development – Turning ideas into strong, memorable logos<br>

🔹 Typography in Logos – Custom lettering and font pairing for unique brand identity<br>

🔹 Minimalism vs. Complexity – Knowing when less is more<br>

🔹 Digital Tools & Techniques – Mastery of Adobe Illustrator and vector graphics<br>
</p>
    
     <h2>Results after course completion</h2>
    <p>
After completing this course, you can work as:  </p>
    <p>
Logo & Brand Identity Designer<br>

Branding Consultant<br>

Creative Director (with experience)<br>

Freelance Designer for global clients<br>

In-house designer for startups & agencies<br>
 </p>
   <div class="img1"> <!--<img src="https://images.unsplash.com/photo-1557683316-973673baf926?auto=format&fit=crop&w=800&q=80" alt="Color swatches and tablet with stylus">--></div>
<h2>What our students say</h2>

<div class="testimonial">
  <div class="avatar">
    <img src="projectimg/auther5.jpeg" alt="Katherine Cutts"/>
  </div>
  <div class="content">
    <div class="stars">★★★★★</div>
    <div class="quote">“This course completely changed my perspective on design. I went from knowing nothing about Photoshop to creating professional logos and branding projects. The portfolio I built during the course helped me land my first freelance client!”</div>
    <div class="name">Katherine Cutts</div>
    <div class="role">Junior Designer at Facebook</div>
  </div>
</div>

<div class="testimonial">
  <div class="avatar">
    <img src="projectimg/auther6.jpg" alt="Daniel Smith"/>
  </div>
  <div class="content">
    <div class="stars">★★★★★</div>
    <div class="quote">"“I joined this course to design a logo for my startup, but I ended up learning so much more. Now I can create scalable, timeless logos that truly represent a brand’s story. Highly recommended!””</div>
    <div class="name">Daniel Smith</div>
    <div class="role">Mobile Developer at Google</div>
  </div>
</div>
  </div>
</div>
</asp:Content>

